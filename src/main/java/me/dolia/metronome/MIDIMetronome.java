...
public class MIDIMetronome implements Metronome {
  ...
  /**
   * Resolution set to 12, because this number can be divided by 1 - quarter notes; 2 - eight notes;
   * 3 - sixteenth notes 4 - eight notes in triples If you change this pattern you must change the
   * track list arrays into createAndSetTrack method!
   */
  ...
  private void createAndSetTrack(int beat, RhythmicPattern pattern)
    ...
    for (int j = 0; j < sample.length; j++) {
      int note = j == 0 && beat > 0 ? LOUD_NOTE
          : QUIET_NOTE;    //if the note is first in sequence, then it is loud note.
      int velocity = j % RESOLUTION == 0 ? 127
          : 90;        //if the note is the first in sequence, then velocity is max
